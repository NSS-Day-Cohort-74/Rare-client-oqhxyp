...
export const UserDetails = ({ token }) => {
  ...
  const didCurrentUserSubscribe = () => {
    if (subscriptions) {
      const userSubscribedToAuthor = subscriptions.find(
        (userSubscription) =>
          userSubscription.follower_id === Number(token) &&
          userSubscription.author_id === userDetails.id
      );
  ...
  const handleUnsubscribe = (event) => {
    event.preventDefault();
    const findUserSubscription = subscriptions.find(
      (subscription) =>
        subscription.follower_id === Number(token) &&
        subscription.author_id === userDetails.id
    );
